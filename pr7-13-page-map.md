...
| Theorem 2.2 structural clause | `gcd(W)=1` 下的 full spectrum / common-coset concentration | `GeneralWeightedGMOStructuralProvider`; `WeightedGMOConcentration`; `sub_mem_addSubgroup_of_common_weightCoset`; `weightedGMOStructuralConclusion_of_stabilizerQuotientProvider`; `generalWeightedGMOSourcePackage_of_aperiodicPackages`; `weightedDifferenceQuotientEquivRange`; `weightedBaseWeightKernelHom_injective`; `generalLemma35Certificate_exists` | `PARTIAL`；逐字全范围接口、共同源/权值陪集、真子群商群回升、群阶归纳、Step 3 的 gcd--torsion 算术及完整 labelled Lemma 3.5 均已核验；Step 1 强 affine 包和 trivial-stabilizer 子群递归仍在闭合，不能把这些基础设施冒充完整定理 |
